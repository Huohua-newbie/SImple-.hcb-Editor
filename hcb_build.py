...
def selset(inputlist):
    ...
    if inputlist[0] == "start":
        ...
        for i in range(1, op_num + 1):
            ...
            return_bytes += b"\x22"
            next_tar = (
                f"::select{str(select_num)}-{str(i + 1)}"
                if i < op_num
                else f"::select{str(select_num)}"
            )
...
cg_loaded = {
    lines[i + 1].strip('"').upper(): int(lines[i], 16) for i in range(0, len(lines), 2)
}
    ...
    if int(inputlist[0]) in bg_list:
        ...
        if len(inputlist) > 1:
            ...
            return_bytes = (
                ...
                + bg_num.to_bytes()
                + b"\x08\x08\x08\x02"
                + bytes.fromhex(function_offset)[::-1]
            )
        else:
            # 第八个入参为-1
            return_bytes = (
                b"\x08\x08\x08\x08\x08\x08\x08\x0c\x01\x19\x08\x08\x02"
                + bytes.fromhex(function_offset)[::-1]
            )
        return_bytes += (
...
def msgset(inputtype):
    ...
    if inputtype == "middle":
        return_bytes = (
            b"\x0c\x01\x0c\x01\x19\x02" + bytes.fromhex(function_offset)[::-1]
        )
...
if __name__ == "__main__":
    ...
    with open("base/base.chb", "rb") as g, open(".test.chb", "wb") as h:
        ...
        h.write(
            oribytes[4 : int(base_off)].replace(
                int(base_off).to_bytes(4, "little"), int(new_off).to_bytes(4, "little")
            )
        )
